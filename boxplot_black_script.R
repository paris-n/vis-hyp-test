...
if(!file.exists("Black_G_Shuffle.csv")){
  ...
  write_csv(G_shuffle, file = "Black_G_Shuffle.csv")
}else{
  G_shuffle <- read_csv("Black_G_Shuffle.csv")
}

results_file <- "Black_lineup_results.csv"
...
run_lineup_test <- function(trial_number, mu_star=0, seed=123) {
  ...
  plot_lineup <- function(data) {
    ggplot(data, aes(x = sample_id, y = value)) +
      geom_boxplot(fill = "Grey", color = "black", alpha = 0.6) +  # Black boxplots
