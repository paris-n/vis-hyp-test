...
if (!file.exists("G_Shuffle_Normality2.csv")) {
  ...
  write_csv(G_shuffle, file = "G_Shuffle_Normality2.csv")
} else {
  G_shuffle <- read_csv("G_Shuffle_Normality2.csv")
}

results_file <- "normality_results2.csv"
...
run_qq_lineup_test <- function(trial_number, df_value, seed=123) {
